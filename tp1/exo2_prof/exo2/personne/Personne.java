...
public class Personne {
	...
	public void marier( Personne fiance) throws ExceptionPersonneDejaMariee, ExceptionPersonnesDeMemeSexe {
		if (( conjoint == null ) && ( sexeMasculin != fiance.sexeMasculin )) {
			...
			if (conjoint != fiance ) {
				//System.err.println( getNomPrenom() + " déjà marié(e)");
				throw new ExceptionPersonneDejaMariee("La personne "+ getNomPrenom() +" est deja mariee");
			}
		}
		else {
			//System.err.println( getNomPrenom() + " et " + fiance.getNomPrenom() + " de même sexe");
			throw new ExceptionPersonnesDeMemeSexe("Les personnes "+ getNomPrenom() 
					+ " et " + fiance.getNomPrenom() + " sont de meme sexe et ne peuvent se marier");
		}
	}
